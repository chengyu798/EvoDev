...
```mermaid
flowchart TD
    START[开始] --> PREPARE[准备工作区]
    PREPARE --> BASELINE[运行基线测试]
    BASELINE --> ANALYZE[分析问题]
    ANALYZE --> IMPLEMENT[实现补丁]
    IMPLEMENT --> TEST[运行修复后测试]
    TEST --> TEST_ROUTE{测试是否通过}
    TEST_ROUTE -->|通过| REVIEW[审查补丁]
    TEST_ROUTE -->|未通过且仍可修复| DIAGNOSE[诊断失败]
    TEST_ROUTE -->|未通过且达到上限| FAIL[记录失败]
    DIAGNOSE --> IMPLEMENT
    REVIEW --> REVIEW_ROUTE{审查是否通过}
    REVIEW_ROUTE -->|通过| EVALUATE[最终评测]
    REVIEW_ROUTE -->|未通过且仍可修复| IMPLEMENT
    REVIEW_ROUTE -->|未通过且达到上限| FAIL
    EVALUATE --> SUCCESS[记录成功]
    SUCCESS --> END[结束]
    FAIL --> END
```
...
| `EVODEV_ARTIFACTS_DIR` | 补丁、日志和测试报告的保存目录 |
