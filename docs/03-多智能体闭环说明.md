...
| `experience.json` | 失败运行生成的结构化经验，仅失败时存在 |
| `experience-retrieval.json` | 经验查询标签、命中特征、原因和最终匹配分 |
| `run-metrics.json` | 工作流与 Agent 版本、Token、耗时、重试和经验使用情况 |

LangGraph 检查点保存到 `EVODEV_DATABASE_URL` 指定的 PostgreSQL 数据库，并使用 `run_id`
作为 `thread_id`，便于后续查询和恢复运行状态。

结构化 Experience 同样保存在 PostgreSQL。新任务会对标签相关性和历史质量加权，过滤低分
候选后检索最多三条经验，并将精简内容与命中依据注入 Analyst 与 Developer。实现和验证细节见
[Experience 经验进化说明](05-经验进化说明.md)。
...
Experience 质量反馈属于主任务结束阶段的轻量操作；Prompt 候选生成和真实 A/B 评测由独立
进程完成，不占用主修复工作流。完整设计见[自进化机制说明](06-自进化机制说明.md)。
