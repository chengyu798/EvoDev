...
DiagnosisRoute = Literal["implement_patch", "finalize_failed"]
...
def route_after_diagnosis(state: EvoDevState) -> DiagnosisRoute:
    if not state.get("failure_retryable", True):
        return "finalize_failed"
    if state.get("iteration", 0) < state["max_iterations"]:
